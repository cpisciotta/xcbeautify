...
final class ParserTests: XCTestCase {
    ...
    func testMatchLDWarningCaptureGroup() throws {
        ...
        let captureGroup = try XCTUnwrap(parser.parse(line: input) as? LDWarningCaptureGroup)
        XCTAssertEqual(captureGroup.warningMessage, "embedded dylibs/frameworks only run on iOS 8 or later")
    }
